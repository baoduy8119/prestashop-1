...
<div class="shopping_cart clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if}">
		...
		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='azblockcart'}" rel="nofollow">
			...
			<span class="text">
				<span class="text_cart">{l s='My Cart' mod='azblockcart'}</span>
			...
			<div class="cart_block block exclusive">
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
							<!--<p class="recent_items {if $products == 0} unvisible {/if}">{l s='There are' mod='azblockcart'} <span class="ajax_cart_quantity">{$cart_qties}</span> <span>item(s)</span> {l s='in your cart' mod='azblockcart'}</p>-->
							<p class="cart_block_no_products{if $products} unvisible{/if}">
								{l s='No product in shopping cart' mod='azblockcart'}
							</p>
												...
												{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
													<li name="customization">
														<div data-id="deleteCustomizableProduct_{$id_customization|intval}_{$product.id_product|intval}_{$product.id_product_attribute|intval}_{$product.id_address_delivery|intval}" class="deleteCustomizableProduct">
															<a class="ajax_cart_block_remove_link" href="{$link->getPageLink("cart", true, NULL, "delete=1&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;token={$static_token}", true)|escape:"html":"UTF-8"}" rel="nofollow"><i class="fa fa-trash"></i></a>
														</div>
						...
						<div class="buttons">
							<a id="button_order_cart" class="btn btn-default button button-small" href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='azblockcart'}" rel="nofollow">
		...
		<div class="layer_inner_box clearfix">
			...
			<div class="layer_inner clearfix">
				<div class="layer_product clearfix mar_b10">
					<div id="pro_added_success" class="alert_title">{l s='Product successfully added to your shopping cart' mod='azblockcart'}</div>
					...
					<div class="layer_product_info">
						...
						<div class="layer_product_infos">
							<span class="layer_cart_label">{l s='Product Atribute' mod='azblockcart'}</span>
							<span id="layer_cart_product_attributes"></span>
						</div>
						
						<div class="layer_product_infos">
							<span class="layer_cart_label">{l s='Product Quantity' mod='azblockcart'}</span>
							<span id="layer_cart_product_quantity"></span>
						</div>
						
						<div class="layer_product_infos">
							<span class="layer_cart_label">{l s='Product Total' mod='azblockcart'}</span>
				...
				<div class="layer_details">
					<div class="layer_cart_row">
						...
						<span class="alert_title ajax_cart_product_txt_s {if $cart_qties < 2} unvisible{/if}">
						...
						<span class="alert_title ajax_cart_product_txt {if $cart_qties > 1} unvisible{/if}">
					...
					<div class="button-container clearfix">	
						<span class="continue button " title="{l s='Continue shopping' mod='azblockcart'}">
							<i class="fa fa-chevron-left left"></i>
							{l s='Continue shopping' mod='azblockcart'}
						</span>
						<a class="button " href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='azblockcart'}" rel="nofollow">
