...
{capture name=path}<a href="{smartblog::GetSmartBlogLink('smartblog')}">{l s='All Blog News' mod='smartblog'}</a>
    ...
    <div id="smartblogcat">
		<div class="smartblogcat-wrap">
			...
			{$blogstyle  = ((isset($smarty.get['AZ_blogOption'])) ? $smarty.get['AZ_blogOption'] : '' )}
			{if !empty($blogstyle)}
				{include file="./$blogstyle.tpl"}
			{elseif isset($AZ_blogOption)}
				{include file="./$AZ_blogOption.tpl"}
			{else}
				{include file="./blog-small_image.tpl"}
			{/if}
