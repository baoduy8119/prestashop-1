...
*  @copyright  2007-2014 AZTemplates SA
...
var ajaxCart = {
	...
	overrideButtonsInThePage : function(){
		...
		$(document).on('click', '.ajax_add_to_cart_button', function(e){
			//$('.header-container .container').css('position','inherit');
			e.preventDefault();
	...
	expand : function(){
		if ($('.cart_block_list').hasClass('collapsed'))
		{
			...
			$.ajax({
				type: 'POST',
				headers: { "cache-control": "no-cache" },
				url: baseDir + 'modules/azblockcart/blockcart-set-collapse.php' + '?rand=' + new Date().getTime(),
				async: true,
				cache: false,
			...
			if (this.id != undefined)
			{
				...
				if ($('dt[data-id="cart_block_product_' + domIdProduct + '"]').length == 0)
				{
					...
					if (typeof(this.is_gift) == 'undefined' || this.is_gift == 0)
						content += '<span class="remove_link"><a rel="nofollow" class="ajax_cart_block_remove_link" href="' + baseUri + '?controller=cart&amp;delete=1&amp;id_product=' + productId + '&amp;token=' + static_token + (this.hasAttributes ? '&amp;ipa=' + parseInt(this.idCombination) : '') + '"> <i class="fa fa-close"></i></a></span>';
					else
						content += '<span class="remove_link"><i class="fa fa-close"></i></span>';
					content += '</dt>';
					if (this.hasAttributes)
