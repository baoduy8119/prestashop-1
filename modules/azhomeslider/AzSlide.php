...
class AzSlide extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public $position;
	public $id_shop;
	public $id_azhomeslider_groups;

	/**
	 * @see ObjectModel::$definition
	 */
	public static $definition = array(
		'table' => 'azhomeslider_slides',
		'primary' => 'id_azhomeslider_slides',
		'multilang' => true,
		'fields' => array(
			'active' =>			array('type' => self::TYPE_BOOL, 'validate' => 'isBool', 'required' => true),
			'position' =>		array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt', 'required' => true),
			'id_azhomeslider_groups' =>		array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt', 'required' => true),
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'azhomeslider` (`id_shop`, `id_azhomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'azhomeslider`
			WHERE `id_azhomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM `'._DB_PREFIX_.'azhomeslider_slides` hss, `'._DB_PREFIX_.'azhomeslider` hs
			WHERE hss.`id_azhomeslider_slides` = hs.`id_azhomeslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_azhomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'azhomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'azhomeslider` hs ON (hss.`id_azhomeslider_slides` = hs.`id_azhomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new AzSlide($row['id_slide']);
