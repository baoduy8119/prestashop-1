...
include_once(_PS_MODULE_DIR_.'azhomeslider/AzSlide.php');
include_once(_PS_MODULE_DIR_.'azhomeslider/AzSlideGroup.php');

class azhomeslider extends Module
{
	protected $_html = '';
	private $default_hook = array( 'displayHomeSlider1', 'displayHomeSlider2', 'displayHomeSlider3', 'displayHomeSlider4' );
		
	public function __construct()
	{
		$this->name = 'azhomeslider';
		...
		$this->displayName = $this->l('AZ Home Slider');
	...
	protected function installSamples()
	{
		$datas = array(
			array(
				'title_module' 		=> 'Home Slider Layout1',
				'position' 			=> 1,
				'active' 			=> 1,
				'display_title_module' 		=> 0,
				'id_azhomeslider_groups' 	=> 1,
				'hook'				=> 'displayHomeSlider1',
				'moduleclass_sfx' 	=> '',
				'autoplay'			=> 0,
		...
		foreach ($datas as $i => $data)
		{
			...
			if($slidegroup->add()){	
				foreach($data['slider'] as $key=>$slide){
					...
					$slider->id_azhomeslider_groups = $slide['id_azhomeslider_groups'];
	...
	protected function createTables()
	{
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'azhomeslider_groups` (
				`id_azhomeslider_groups` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`position` int(10) unsigned NOT NULL DEFAULT \'0\',
				`active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
				`hook` int(10) unsigned, 
				`params` text NOT NULL DEFAULT \'\' ,			  
			  PRIMARY KEY (`id_azhomeslider_groups`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');		
		/* Group Shop*/
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'azhomeslider_groups_shop` (
				`id_azhomeslider_groups` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_shop` int(10) unsigned NOT NULL,
				PRIMARY KEY (`id_azhomeslider_groups`, `id_shop`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');			
		/* Groups lang */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'azhomeslider_groups_lang` (
			  `id_azhomeslider_groups` int(10) unsigned NOT NULL,
			  `id_lang` int(10) unsigned NOT NULL,
			  `title` varchar(255) NOT NULL,
			  PRIMARY KEY (`id_azhomeslider_groups`,`id_lang`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');		
		/* Slides */
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'azhomeslider` (
				`id_azhomeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_shop` int(10) unsigned NOT NULL,
				PRIMARY KEY (`id_azhomeslider_slides`, `id_shop`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'azhomeslider_slides` (
				`id_azhomeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_azhomeslider_groups` int(10) unsigned NOT NULL,
				`position` int(10) unsigned NOT NULL DEFAULT \'0\',
				`active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',			  
			  PRIMARY KEY (`id_azhomeslider_slides`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides lang configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'azhomeslider_slides_lang` (
			  `id_azhomeslider_slides` int(10) unsigned NOT NULL,
			  `id_lang` int(10) unsigned NOT NULL,
			  `title` varchar(255) NOT NULL,
			  ...
			  PRIMARY KEY (`id_azhomeslider_slides`,`id_lang`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new AzSlide($slide['id_slide']);
			$to_del->delete();
		}

		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'azhomeslider`, `'._DB_PREFIX_.'azhomeslider_slides`, `'._DB_PREFIX_.'azhomeslider_slides_lang`, `'._DB_PREFIX_.'azhomeslider_groups`, `'._DB_PREFIX_.'azhomeslider_groups_shop`, `'._DB_PREFIX_.'azhomeslider_groups_lang`;
		');
	}
	...
	public function getContent()
	{
		...
		)
		{
			if ($this->_postValidation())
			{
				...
				if(Tools::getValue('id_azhomeslider_groups'))
		...
		elseif (Tools::isSubmit('addSlide') || (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide'))))
		{
			...
			{
				$associated_shop_ids = AzSlide::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
	...
	protected function _postProcess()
	{
		...
		if (Tools::isSubmit('submitGroups') || Tools::isSubmit('submitSaveAndStayGroup'))
		{
			if (Tools::getValue('id_azhomeslider_groups'))
			{
				$spgoup = new AzSlideGroup((int)Tools::getValue('id_azhomeslider_groups'));
			...
			if (!Tools::getValue('id_azhomeslider_groups'))
			{
				...
				else
				{
					if(Tools::isSubmit('submitSaveAndStayGroup'))
						Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=6&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&editGroup&id_azhomeslider_groups='.$spgoup->id);
					else	
		...
		elseif (Tools::isSubmit('changeStatusGroup') && Tools::isSubmit('id_azhomeslider_groups'))
		{
			$slidegroup = new AzSlideGroup((int)Tools::getValue('id_azhomeslider_groups'));
		...
		elseif (Tools::isSubmit('duplicateGroup') && Tools::isSubmit('id_azhomeslider_groups'))
		{
			$id_azhomeslider_groups = Tools::getValue('id_azhomeslider_groups');
			$slidegroup = new AzSlideGroup((int)Tools::getValue('id_azhomeslider_groups'));
			$slidegroup->active = 0;
			$slidegroup->id = null;
			
			if($slidegroup->add()){
				$sliders = $this->getIdSliderByGroup($id_azhomeslider_groups);
				if($sliders){
					foreach($sliders as $slider){
						$new_slider = new Azslide((int)$slider['id_azhomeslider_slides']);
						$new_slider->id = null;
						$new_slider->id_azhomeslider_groups = $slidegroup->id;
						$new_slider->add();
					}
				}
			}
		...
		{
			$slide = new AzSlide((int)Tools::getValue('id_slide'));
			if ($slide->active == 0)
				$slide->active = 1;
			else
				$slide->active = 0;
			$res = $slide->update();
			$this->clearCache();
			Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=6&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&editGroup&id_azhomeslider_groups='.Tools::getValue('id_azhomeslider_groups'));
		}
		/* Processes Slide */
		elseif (Tools::isSubmit('submitSlide'))
		{
			...
			$slide->active = (int)Tools::getValue('active_slide');
						/* Sets active */
			$slide->id_azhomeslider_groups = (int)Tools::getValue('id_azhomeslider_groups');
		...
		{
			$slidegroup = new AzSlideGroup((int)Tools::getValue('delete_id_group'));
		...
		{
			$slide = new AzSlide((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			$this->clearCache();
			if (!$res)
				$this->_html .= $this->displayError('Could not delete.');
			else
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&editGroup&id_azhomeslider_groups='.Tools::getValue('id_azhomeslider_groups'));
		}
	...
	protected function _prepareHook($hook='displayHome')
	{
		...
		$smarty_cache_id = $this->getCacheId ('azhomeslider_'.$hook);
		if (!$this->isCached ('azhomeslider.tpl', $smarty_cache_id))
		{
			...
			if($slides[0]['id_azhomeslider_groups'])
				$id_azhomeslider_groups = $slides[0]['id_azhomeslider_groups'];

			$this->context->smarty->assign (array(
				'azhomeslider_slides' 		=> $slides,
				'params'					=> $params,
				'id_azhomeslider_groups'	=> $id_azhomeslider_groups,
	...
	{
		$this->context->controller->addCSS($this->_path.'css/azhomeslider.css');
	...
	{
		$smarty_cache_id = $this->getCacheId ('azhomeslider_displayHomeSlider1');
		if (!$this->_prepareHook('displayHomeSlider1'))
			return false;

		return $this->display (__FILE__, 'azhomeslider.tpl', $smarty_cache_id);
	}
	
	public function hookdisplayHomeSlider2()
	{
		$smarty_cache_id = $this->getCacheId ('azhomeslider_displayHomeSlider2');
	...
	{
		$smarty_cache_id = $this->getCacheId ('azhomeslider_displayHomeSlider3');
	...
	public function hookdisplayHomeSlider4()
	{
		$smarty_cache_id = $this->getCacheId ('azhomeslider_displayHomeSlider4');
	...
	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'azhomeslider (id_azhomeslider_slides, id_shop)
			SELECT id_azhomeslider_slides, '.(int)$params['new_id_shop'].'
			FROM '._DB_PREFIX_.'azhomeslider
	...
	public function getNextPositionGroup()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			FROM `'._DB_PREFIX_.'azhomeslider_groups` hssg'
		);

		return (++$row['next_position']);
	}

	public function getSlides($active = null,$id_azhomeslider_groups = 0,$id_hook=0)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_azhomeslider_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
			hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`,hssg.`params`,hssg.`id_azhomeslider_groups`,hsgl.`title` as title_group
			FROM '._DB_PREFIX_.'azhomeslider hs
			LEFT JOIN '._DB_PREFIX_.'azhomeslider_slides hss ON (hs.id_azhomeslider_slides = hss.id_azhomeslider_slides)
			LEFT JOIN '._DB_PREFIX_.'azhomeslider_slides_lang hssl ON (hss.id_azhomeslider_slides = hssl.id_azhomeslider_slides)
			LEFT JOIN '._DB_PREFIX_.'azhomeslider_groups hssg ON (hss.id_azhomeslider_groups = hssg.id_azhomeslider_groups)
			LEFT JOIN '._DB_PREFIX_.'azhomeslider_groups_lang hsgl ON (hssg.id_azhomeslider_groups = hsgl.id_azhomeslider_groups)
			WHERE id_shop = '.(int)$id_shop.'
			'.($id_azhomeslider_groups != 0 ? ' AND hss.`id_azhomeslider_groups` = '.(int)$id_azhomeslider_groups.'' : ' ').'
	...
	public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
	{
		...
		$results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN '._DB_PREFIX_.'azhomeslider_slides hss ON (hs.id_azhomeslider_slides = hss.id_azhomeslider_slides)
			LEFT JOIN '._DB_PREFIX_.'azhomeslider_slides_lang hssl ON (hss.id_azhomeslider_slides = hssl.id_azhomeslider_slides)
			WHERE hs.`id_azhomeslider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
			($active ? ' AND hss.`active` = 1' : ' ')
		);
	...
	public function displayStatus($id_slide, $active)
	{
		...
		$html = '<a class="btn '.$class.'" href="'.AdminController::$currentIndex.
			'&configure='.$this->name.'
				...
				&changeStatus&id_slide='.(int)$id_slide.'&id_azhomeslider_groups='.Tools::getValue('id_azhomeslider_groups').'" title="'.$title.'"><i class="'.$icon.'"></i> '.$title.'</a>';
	...
	{
		$req = 'SELECT hs.`id_azhomeslider_slides` as id_slide
				FROM `'._DB_PREFIX_.'azhomeslider` hs
				WHERE hs.`id_azhomeslider_slides` = '.(int)$id_slide;
	...
	{
		$id_azhomeslider_groups = Tools::getValue('id_azhomeslider_groups') ? Tools::getValue('id_azhomeslider_groups') : 0 ; 
		$slides = $this->getSlides(null,$id_azhomeslider_groups);
		foreach ($slides as $key => $slide)
		{
			$slides[$key]['status'] = $this->displayStatus($slide['id_slide'], $slide['active']);
			$associated_shop_ids = AzSlide::getAssociatedIdsShop((int)$slide['id_slide']);
	...
	public function renderAddForm()
	{
			...
			'form' => array(
				...
				'input' => array(
					...
					array(
						'type' => 'select',
						'label' => $this->l('Group'),
						'name' => 'id_azhomeslider_groups',
						'options' => array(  'query' => $modules,
						'id' => 'id_azhomeslider_groups',
						'name' => 'title' ),
					 ),						
					array(
						'type' => 'text',
		...
		$helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_azhomeslider_groups='.Tools::getValue('id_azhomeslider_groups');
		...
		'form' => array(
			...
			'input' => array(
				...
				array(
					...
					'hint'   => $this->l('Allow to on/off title of module.'),
				...
				array(
					...
					'hint'   => $this->l('Display module in this Hook selected.'),
					'name' => 'hook',
	...
	public function getConfigFieldsValues()
	{
		...
		if (Tools::isSubmit('id_azhomeslider_groups'))
		{
			$spgroup = new AzSlideGroup((int)Tools::getValue('id_azhomeslider_groups'));
			$fields['id_azhomeslider_groups'] = (int)Tools::getValue('id_azhomeslider_groups', $spgroup->id);
		}
		else
			$spgroup = new AzSlideGroup();
	...
	private function displayForm()
	{
		...
		else if (Shop::getContext() != Shop::CONTEXT_GROUP && Shop::getContext() != Shop::CONTEXT_ALL)
		{
			...
			if (!empty($modules))
			{
				...
				{
					$associated_shop_ids = AzSlideGroup::getAssociatedIdsShop((int)$mod['id_azhomeslider_groups']);
					if ($associated_shop_ids && count($associated_shop_ids) > 1)
		...
		$this->_html .= '
	 	<div class="panel">
			...
			<tr class="nodrag nodrop">
				...
				<th class=" left">'.$this->l('Motule Title').'</th>
		...
		if (!empty($modules))
		{
			...
			{
				$this->_html .= '
				<tr id="item_'.$spgroup['id_azhomeslider_groups'].'" class=" '.( $irow ++ % 2?' ':'' ).'">
					<td class=" 	" onclick="document.location = \''.$currentIndex.'&configure='.$this->name.'&token='
					.Tools::getAdminTokenLite ('AdminModules').'&editGroup&id_azhomeslider_groups='
					.$spgroup['id_azhomeslider_groups'].'\'">'
					.$spgroup['id_azhomeslider_groups'].'</td>
					...
					.'&editGroup&id_azhomeslider_groups='.$spgroup['id_azhomeslider_groups'].'\'">'.$spgroup['title']
					...
					.'&token='.Tools::getAdminTokenLite ('AdminModules').'&editGroup&id_azhomeslider_groups='
					.$spgroup['id_azhomeslider_groups'].'\'">'
					.( Validate::isInt ($spgroup['hook'])?$this->getHookTitle ($spgroup['hook']):'' ).'</td>
					
					<td class="  "> <a href="'.$currentIndex.'&configure='.$this->name.'&token='
					.Tools::getAdminTokenLite ('AdminModules')
					.'&changeStatusGroup&id_azhomeslider_groups='.$spgroup['id_azhomeslider_groups'].'&hook='.$spgroup['hook'].'">'.( $spgroup['active']?'
					<i class="icon-check"></i>':'<i class="icon-remove"></i>' ).'</a> </td>
					
					<td class="text-right">
						<div class="btn-group-action">
							<div class="btn-group pull-right">
								<a class="btn btn-default" href="'.$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite ('AdminModules').'&editGroup&id_azhomeslider_groups='.$spgroup['id_azhomeslider_groups'].'">
					...
					.$this->name.'&token='
					.Tools::getAdminTokenLite ('AdminModules').'&duplicateGroup&id_azhomeslider_groups='
					.$spgroup['id_azhomeslider_groups'].'">
	...
	private function getGridItems()
	{
		...
		$id_lang = $this->context->language->id;
		$id_shop = $this->context->shop->id;
		$sql = 'SELECT b.`id_azhomeslider_groups`,  b.`hook`, b.`position`, b.`active`, bl.`title`
			FROM `'._DB_PREFIX_.'azhomeslider_groups` b
			LEFT JOIN `'._DB_PREFIX_.'azhomeslider_groups_shop` bs ON (b.`id_azhomeslider_groups` = bs.`id_azhomeslider_groups` )
			LEFT JOIN `'._DB_PREFIX_.'azhomeslider_groups_lang` bl ON (b.`id_azhomeslider_groups` = bl.`id_azhomeslider_groups`)
	...
	public function getAddFieldsValues()
	{
		...
		$fields['id_azhomeslider_groups'] = Tools::getValue('id_azhomeslider_groups', $slide->id_azhomeslider_groups);
	...
	private function getIdSliderByGroup($id_azhomeslider_groups){
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT ss.`id_azhomeslider_slides`
			FROM `'._DB_PREFIX_.'azhomeslider_slides` ss
			WHERE ss.`id_azhomeslider_groups` = '.(int)$id_azhomeslider_groups
		);
		return $result;
	}
	...
	{
		$sql = 'SELECT MAX(`id_azhomeslider_groups`)
				FROM `'._DB_PREFIX_.'azhomeslider_groups`';
		$azhomeslider_groups = DB::getInstance ()->getValue($sql);
		return ( is_numeric ($azhomeslider_groups) )?$azhomeslider_groups:1;
	}
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			FROM `'._DB_PREFIX_.'azhomeslider_groups` cs, `'._DB_PREFIX_.'azhomeslider_groups_shop` css
			WHERE css.`id_azhomeslider_groups` = cs.`id_azhomeslider_groups` AND css.`id_shop` = '.(int)$this->context->shop->id
		);
	...
	{
		$this->_clearCache('azhomeslider.tpl');
	}
}
