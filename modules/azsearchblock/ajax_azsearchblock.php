...
$azsearchblock = new AzSearchBlock();
$items = array();
if (!Tools::isSubmit ('secure_key') || Tools::getValue ('secure_key') != $azsearchblock->secure_key || !Tools::getValue ('action'))
	die( 1 );
...
if (Tools::getValue ('action') == 'updateSlidesPosition' && Tools::getValue ('item'))
{
	...
	foreach ($items as $position => $item)
	{
		$success = Db::getInstance ()->execute ('
			UPDATE `'._DB_PREFIX_.'azsearchblock` SET `ordering` = '.(int)$position.'
			WHERE `azsearchblock` = '.(int)$item);
		$pos[] = $position;
	}
