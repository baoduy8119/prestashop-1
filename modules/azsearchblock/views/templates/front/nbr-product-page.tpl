...
{if isset($p) AND $p}
	...
	{else}
        ...
        {*{assign var='requestPage' value=$link->getModuleLink('spsearchpro','catesearch')|addslashes}*}
        {*{assign var='requestNb_module' value=$link->getModuleLink('spsearchpro','catesearch')|addslashes}*}
	{/if}
    {*{$requestNb.requestUrl = $requestNb.requestUrl|cat:'&'}*}
    {*{$requestNb.controller = 'catesearch'}*}

    {*{$requestNb.requestUrl = $requestNb_module}*}
	<!-- nbr product/page -->
	{if $nb_products > $n_array[0]}
		<form action="{$link->getModuleLink('spsearchpro','catesearch')|addslashes}" method="get" class="nbrItemPage">
			<div class="clearfix selector1">
				...
				<span>{l s='per page' mod='spsearchpro'}</span>
			</div>
		</form>
	{/if}
	<!-- /nbr product/page -->
