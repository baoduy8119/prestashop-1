...
class azBlocknewsletter extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('AZ Newsletter Block');
	...
	{
		if (!parent::install() || !$this->registerHook(array('header', 'displayNewLetter','displayNewLetterPopup', 'actionCustomerAccountAdd')))
			return false;
	...
	public function hookdisplayNewLetter($params)
	{
	 ...
	 {
		 if(isset($_COOKIE['az_newsletter']))
			 return	;
		 if (!isset($this->prepared) || !$this->prepared)
			 $this->_prepareHook($params);
		 $this->prepared = true;
	...
	{
		$this->context->controller->addCSS($this->_path.'azblocknewsletter.css', 'all');
