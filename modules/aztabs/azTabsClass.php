...
class azTabsClass extends ObjectModel
{
	public $id_aztabs;
	public $title_module;
	public $short_desc;
	...
	{
		Shop::addTableAssociation ('aztabs', array('type' => 'shop'));
	...
	{
		$sql = 'SELECT MAX(`id_aztabs`)
				FROM `'._DB_PREFIX_.'aztabs`';
		$id_aztabs = DB::getInstance ()->getValue ($sql);
		return ( is_numeric ($id_aztabs) )?$id_aztabs:1;
	}
	...
	public static function getAssociatedIdsShop($id_module)
	{
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN `'._DB_PREFIX_.'aztabs_shop` css ON (css.`id_aztabs` = cs.`id_aztabs`)
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM `'._DB_PREFIX_.'aztabs` cs, `'._DB_PREFIX_.'az_shop` css
			WHERE css.`id_aztabs` = cs.`id_aztabs` AND css.`id_shop` = '.(int)$id_shop
		);

		if ((int)$max == (int)$id_aztabs)
			return true;
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT cs.`ordering` as ordering, cs.`id_aztabs` as id_aztabs
			FROM `'._DB_PREFIX_.'aztabs` cs
			LEFT JOIN `'._DB_PREFIX_.'aztabs_shop` css ON (css.`id_aztabs` = cs.`id_aztabs`)
		...
		foreach ($rows as $row)
		{
			$customs = new SpListingTabsClass($row['id_aztabs']);
			--$customs->ordering;
			$customs->update();
			unset($customs);
		}
