...
include_once(_PS_MODULE_DIR_.'azmegamenu/Megamenu.php');
include_once(_PS_MODULE_DIR_.'azmegamenu/MegamenuGroup.php');

class azmegamenu extends Module
{
	protected $_html = '';
	protected $azmegamenu_style = 'mega';
	private $default_hook = array( 'displayMenu', 'displayMenu2' );	
	public function __construct()
	{
		$this->name = 'azmegamenu';
        ...
        $this->displayName = $this->l('AZ Mega Menu');
        $this->description = $this->l('Adds a new horizontal mega menu to the top of your website.');
	...
	public function install()
	{
            ...
            $this->registerHook('actionShopDataDuplication'))
		{
			...
			foreach ($shops as $shop_id)
			{
				...
				$res = Configuration::updateValue('azmegamenu_style', $this->azmegamenu_style, false, $shop_group_id, $shop_id);
			}
			...
			if (count($shop_groups_list))
			{
				...
				{
					$res = Configuration::updateValue('azmegamenu_style', $this->azmegamenu_style, false, $shop_group_id);
				}
			}

			/* Sets up Global configuration */
			$res = Configuration::updateValue('azmegamenu_style', $this->azmegamenu_style);	
			/* Creates tables */
	...
	public function uninstall()
	{
		...
		if (parent::uninstall())
		{
			...
			$res &= Configuration::deleteByName('azmegamenu_style');
			return (bool)$res;
		}
	...
	{
		/* azmegamenu */
		$res = Db::getInstance ()->Execute ('DROP TABLE IF EXISTS `'._DB_PREFIX_.'azmegamenu_group`')
			&& Db::getInstance ()->Execute ('CREATE TABLE `'._DB_PREFIX_.'azmegamenu_group` (
			`id_azmegamenu_group` int(10) unsigned NOT NULL AUTO_INCREMENT,
			`hook` varchar(20) NOT NULL, 
			`params` text NOT NULL DEFAULT \'\' ,
			`status` tinyint(1) NOT NULL DEFAULT \'1\',
			`position` int(10) unsigned NOT NULL,
			PRIMARY KEY (`id_azmegamenu_group`)) ENGINE=InnoDB default CHARSET=utf8');
		$res &= Db::getInstance ()->Execute ('DROP TABLE IF EXISTS `'._DB_PREFIX_.'azmegamenu_group_shop`')
			&& Db::getInstance ()->Execute ('CREATE TABLE `'._DB_PREFIX_.'azmegamenu_group_shop` (
			`id_azmegamenu_group` int(10) unsigned NOT NULL,
			`id_shop` int(10) unsigned NOT NULL, 
			PRIMARY KEY (`id_azmegamenu_group`,`id_shop`)) ENGINE=InnoDB default CHARSET=utf8');
		$res &= Db::getInstance ()->Execute ('DROP TABLE IF EXISTS `'._DB_PREFIX_.'azmegamenu_group_lang`')
			&& Db::getInstance ()->Execute ('CREATE TABLE '._DB_PREFIX_.'azmegamenu_group_lang (
			`id_azmegamenu_group` int(10) unsigned NOT NULL,
			`id_lang` int(10) unsigned NOT NULL,
			`title` varchar(255) NOT NULL DEFAULT \'\',
			`content` text,
			PRIMARY KEY (`id_azmegamenu_group`,`id_lang`)) ENGINE=InnoDB default CHARSET=utf8');
			
		$res &= (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'azmegamenu` (
				`id_azmegamenu` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_azmegamenu_group` int(10) unsigned NOT NULL,
				...
				`az_lesp` int(11) NOT NULL,
				`position` int(11) NOT NULL,
				`active` tinyint(1) NOT NULL,
			  PRIMARY KEY (`id_azmegamenu`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* azmegamenu lang */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'azmegamenu_lang` (
			  `id_azmegamenu` int(11) NOT NULL,
			  ...
			  PRIMARY KEY (`id_azmegamenu`,`id_lang`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8;
		');

		/* menus shop */
		$res &= Db::getInstance()->execute('CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'azmegamenu_shop` (
			  `id_azmegamenu` int(11) NOT NULL,
			  `id_shop` int(11) NOT NULL,
			  PRIMARY KEY (`id_azmegamenu`,`id_shop`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8;
		');
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new Megamenu($menu['id_azmegamenu']);
			$to_del->delete();
		}

		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS 
				`'._DB_PREFIX_.'azmegamenu`, 
				`'._DB_PREFIX_.'azmegamenu_lang`, 
				`'._DB_PREFIX_.'azmegamenu_shop`,
				`'._DB_PREFIX_.'azmegamenu_group`,
				`'._DB_PREFIX_.'azmegamenu_group_shop`,
				`'._DB_PREFIX_.'azmegamenu_group_lang`;
		');
	}
	...
	public function getContent()
	{
		...
		/* Validate & process */
		if (Tools::isSubmit('submitMenu') || Tools::isSubmit('delete_id_azmegamenu') || Tools::isSubmit('duplicate_id_azmegamenu') ||
	...
	protected function _postValidation()
	{
		...
		{
			if (!Validate::isInt(Tools::getValue('id_azmegamenu')))
		...
		elseif (Tools::isSubmit('submitMenu'))
		{
			...
			/* If edit : checks id_azmegamenu */
			if (Tools::isSubmit('id_azmegamenu'))
			{

				//d(var_dump(Tools::getValue('id_azmegamenu')));
				if (!Validate::isInt(Tools::getValue('id_azmegamenu')) && !$this->menuExists(Tools::getValue('id_azmegamenu')))
		...
		elseif (Tools::isSubmit('delete_id_azmegamenu') && (!Validate::isInt(Tools::getValue('delete_id_azmegamenu')) || !$this->menuExists((int)Tools::getValue('delete_id_azmegamenu'))))
			$errors[] = $this->l('Invalid menu ID');
	...
	protected function _postProcess()
	{
		...
		if (Tools::isSubmit('submitConfigMenu'))
		{
			...
			foreach ($shops as $shop_id)
			{
				...
				$res = Configuration::updateValue('azmegamenu_style', Tools::getValue('azmegamenu_style'), false, $shop_group_id, $shop_id);
			}
			...
			switch ($shop_context)
			{
				case Shop::CONTEXT_ALL:
					$res = Configuration::updateValue('azmegamenu_style', Tools::getValue('azmegamenu_style'));
					if (count($shop_groups_list))
					{
						foreach ($shop_groups_list as $shop_group_id)
						{
							$res = Configuration::updateValue('azmegamenu_style', Tools::getValue('azmegamenu_style'), false, $shop_group_id);
						}
					}
					break;
		...
		elseif (Tools::isSubmit('changeStatusMenuGroup') && Tools::getValue('id_azmegamenu_group'))
		{
			$menugroup = new MegamenuGroup((int)Tools::getValue('id_azmegamenu_group'));
		...
		elseif (Tools::isSubmit('duplicateMenuGroup') && Tools::isSubmit('id_azmegamenu_group'))
		{
			$menugroup = new MegamenuGroup((int)Tools::getValue('id_azmegamenu_group'));
			...
			if($menugroup->add()){
				$menus = $this->getIdMenuByGroup($id_azmegamenu_group,true);
				if($menus){
					foreach($menus as $menu){
						$new_menu = new Megamenu((int)$menu['id_azmegamenu']);
						$new_menu->id = null;
						$new_menu->id_parent = 1;
						$new_menu->id_azmegamenu_group = $menugroup->id;
						$new_menu->position = $new_menu->getmaxPositonMenu();
						$id_parent = (int)$menu['id_azmegamenu'];
						if($new_menu->add())
		...
		elseif (Tools::isSubmit('deleteMenuGroup'))
		{
			$menugroup = new MegamenuGroup((int)Tools::getValue('id_azmegamenu_group'));
			if($menugroup->delete()){
				$menus = $this->getIdMenuByGroup((int)Tools::getValue('id_azmegamenu_group'));
				if($menus){
					foreach($menus as $menu){
						$new_menu = new Megamenu((int)$menu['id_azmegamenu']);
						$new_menu->delete();
		...
		elseif (Tools::isSubmit('saveAndStayGroup') || Tools::isSubmit('saveGroupMenu'))
		{
			...
			if (Tools::getValue('id_azmegamenu_group'))
			{
				$menugroup = new MegamenuGroup((int)Tools::getValue ('id_azmegamenu_group'));
			...
			Tools::getValue ('id_azmegamenu_group') && $this->moduleExists((int)Tools::getValue ('id_azmegamenu_group'))  ? $menugroup->update() : $menugroup->add ();
			$this->clearCache();
			if (Tools::isSubmit ('saveAndStayGroup'))
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=4&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&editMenugroup&id_azmegamenu_group='.$menugroup->id);
			else
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=4&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);				
			}
		}	
			elseif (Tools::isSubmit('changeStatus') && Tools::isSubmit('id_azmegamenu'))
		{
			$menu = new Megamenu((int)Tools::getValue('id_azmegamenu'));
			if ($menu->active == 0)
				$menu->active = 1;
			else
				$menu->active = 0;
			$res = $menu->update();
			$this->clearCache();
			Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=4&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&editMenugroup&id_azmegamenu_group='.(int)Tools::getValue('id_azmegamenu_group'));
		}
		...
		{
			/* Sets ID if needed */
			if (Tools::getValue('id_azmegamenu'))
			{
				$mgmenu = new Megamenu((int)Tools::getValue('id_azmegamenu'));
			...
			$mgmenu->id_azmegamenu_group  = (int)Tools::getValue('id_azmegamenu_group');			
			/* Sets type */
		...
		elseif (Tools::isSubmit('delete_id_azmegamenu'))
		{
			Megamenu::deleteMenu((int)Tools::getValue('delete_id_azmegamenu'));
			$this->clearCache();
			Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&editMenugroup&id_azmegamenu_group='.(int)Tools::getValue('id_azmegamenu_group'));
		}
		elseif (Tools::isSubmit('duplicate_id_azmegamenu'))
		{
			$id_parent = (int)Tools::getValue('duplicate_id_azmegamenu');
			$id_azmegamenu_group = (int)Tools::getValue('id_azmegamenu_group');
			$parent_menu = new Megamenu((int)Tools::getValue('duplicate_id_azmegamenu'));
			$parent_menu->id = null;
			$parent_menu->id_azmegamenu = null;
			$parent_menu->active = 0;
			$parent_menu->position = $parent_menu->getmaxPositonMenu();
			if ($parent_menu->add())
				$this->duplicateParentMenu($id_parent,$parent_menu->id,$id_azmegamenu_group);
			Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=19&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&editMenugroup&id_azmegamenu_group='.(int)Tools::getValue('id_azmegamenu_group'));
		}		

		/* Display errors if needed */
		if (count($errors))
			$this->_html .= $this->displayError(implode('<br />', $errors));
		elseif (Tools::isSubmit('submitMenu') && Tools::getValue('id_azmegamenu'))
			Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=4&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_azmegamenu='.Tools::getValue('id_azmegamenu').'&editMenugroup&id_azmegamenu_group='.(int)Tools::getValue('id_azmegamenu_group'));
		elseif (Tools::isSubmit('submitMenu'))
			Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&conf=3&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&editMenugroup&id_azmegamenu_group='.(int)Tools::getValue('id_azmegamenu_group'));
	}

	protected function _prepareHook($hook)
	{
		if (!$this->isCached('azmegamenu.tpl', $this->getCacheId()))
		{
			global $link;
			if(!$hook)
				return;
			...
			$azmegamenu_style = Tools::getValue('azmegamenu_style', Configuration::get('azmegamenu_style', null, $id_shop_group, $id_shop));
			$current_link = $link->getPageLink('', false, $this->context->language->id);
			$object = new Megamenu();
			$groups 	= $this->getIdGroupByHook($hook);
			
			if(isset($groups['id_azmegamenu_group']) && $groups['id_azmegamenu_group'])
				$megamenu = $object->getMegamenu(1, 1,$groups['id_azmegamenu_group']);
			else
				$megamenu = '';
			$this->smarty->assign( 'azmegamenu', $megamenu );
			$this->smarty->assign( 'current_link', $current_link );
			$this->smarty->assign( 'azmegamenu_style', $azmegamenu_style );
			return $this->display(__FILE__,'azmegamenu.tpl');
		}
    ...
    {
		$this->context->controller->addCss( __PS_BASE_URI__.'modules/azmegamenu/css/azmegamenu.css' ); 
    }
	public function hookdisplayMenu($params)
	{
		...
		return $this->display(__FILE__, 'azmegamenu.tpl', $this->getCacheId());
	}
	...
	public function hookdisplayMenu2($params)
	{
		...
		return $this->display(__FILE__, 'azmegamenu2.tpl', $this->getCacheId());
	}
	...
	{
		$this->_clearCache('azmegamenu.tpl');
	}
	
	private function getIdGroupByHook($hook){
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
				SELECT mg.`id_azmegamenu_group`
				FROM `'._DB_PREFIX_.'azmegamenu_group` mg
	...
	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'azmegamenu (id_azmegamenu, id_shop)
			SELECT id_azmegamenu, '.(int)$params['new_id_shop'].'
					...
					update: function() {
						var order = $(this).sortable("serialize") + "&action=updateGroupPosition";
							$.ajax({
								...
								dataType: "json",
								data:order,
								url:"'._PS_BASE_URL_.__PS_BASE_URI__.'modules/'.$this->name.'/ajax_azsortgroup.php?secure_key='.$this->secure_key.'",
	...
	public function getMenus($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_azmegamenu` as id_azmegamenu, hs.`position`, hs.`active`,hs.`type`, hssl.`title`
			FROM '._DB_PREFIX_.'azmegamenu hs
			LEFT JOIN '._DB_PREFIX_.'azmegamenu_shop hss ON (hs.id_azmegamenu = hss.id_azmegamenu)
			LEFT JOIN '._DB_PREFIX_.'azmegamenu_lang hssl ON (hss.id_azmegamenu = hssl.id_azmegamenu)
	...
	public function menuExists($id_azmegamenu)
	{
		$req = 'SELECT hs.`id_azmegamenu` as id_azmegamenu
				FROM `'._DB_PREFIX_.'azmegamenu` hs
	...
	public function initForm()
	{
		...
		$helper->module = $this;
		$helper->name_controller = 'azmegamenu_group';
		...
		if (Tools::isSubmit ('id_azmegamenu_group') && $id_azmegamenu_group)
		{
			$azmenugroup = new MegamenuGroup((int)$id_azmegamenu_group);
			$params = unserialize($azmenugroup->params);
			$this->fields_form[0]['form']['input'][] = array(
				'type' => 'hidden',
				'name' => 'id_azmegamenu_group' );
		$helper->fields_value['id_azmegamenu_group'] = Tools::getValue ('id_azmegamenu_group', $azmenugroup->id);
		}
		else
		{
			$azmenugroup = new MegamenuGroup();
			$params = array();
		}
		foreach (Language::getLanguages (false) as $lang)
		{
			$helper->fields_value['title'][(int)$lang['id_lang']] = Tools::getValue ('title_'
				.(int)$lang['id_lang'],
				$azmenugroup->title[(int)$lang['id_lang']]);
		}

		$helper->fields_value['hook'] = Tools::getValue ('hook', $azmenugroup->hook);
		$helper->fields_value['status'] = (int)Tools::getValue('status', $azmenugroup->status);
	...
	private function displayForm()
	{
		...
		else if (Shop::getContext() != Shop::CONTEXT_GROUP && Shop::getContext() != Shop::CONTEXT_ALL)
		{
			...
			if (!empty($modules))
			{
				...
				{
					$associated_shop_ids = MegamenuGroup::getAssociatedIdsShop((int)$mod['id_azmegamenu_group']);
		...
		{
			static $irow;
			foreach ($modules as $azmenugroup)
			{
				$this->_html .= '
				<tr id="item_'.$azmenugroup['id_azmegamenu_group'].'" class=" '.( $irow ++ % 2?' ':'' ).'">
					<td class=" 	" onclick="document.location = \''.$currentIndex.'&configure='.$this->name.'&token='
					.Tools::getAdminTokenLite ('AdminModules').'&editMenugroup&id_azmegamenu_group='
					.$azmenugroup['id_azmegamenu_group'].'\'">'
					.$azmenugroup['id_azmegamenu_group'].'</td>
					<td class=" dragHandle"><div class="dragGroup"><div class="positions">'.$azmenugroup['position']
					.'</div></div></td>
					<td class="  " onclick="document.location = \''.$currentIndex.'&configure='.$this->name.'&token='
					.Tools::getAdminTokenLite ('AdminModules')
					.'&editMenugroup&id_azmegamenu_group='.$azmenugroup['id_azmegamenu_group'].'\'">'.$azmenugroup['title']
					.' '.($azmenugroup['is_shared'] ? '<span class="label color_field"
		style="background-color:#108510;color:white;margin-top:5px;">'.$this->l('Shared').'</span>' : '').'</td>
					...
					.'&token='.Tools::getAdminTokenLite ('AdminModules').'&editMenugroup&id_azmegamenu_group='
					.$azmenugroup['id_azmegamenu_group'].'\'">'
					.$azmenugroup['hook'].'</td>
					<td class="  "> <a href="'.$currentIndex.'&configure='.$this->name.'&token='
					.Tools::getAdminTokenLite ('AdminModules')
					.'&changeStatusMenuGroup&id_azmegamenu_group='.$azmenugroup['id_azmegamenu_group'].'&status='
					.$azmenugroup['active'].'&hook='.$azmenugroup['hook'].'">'.( ($azmenugroup['status'] && $azmenugroup['status'] == 1)?'
		...
		.Tools::getAdminTokenLite ('AdminModules').'&editMenugroup&id_azmegamenu_group='.$azmenugroup['id_azmegamenu_group'].'">
					...
					.$this->name.'&token='
					.Tools::getAdminTokenLite ('AdminModules').'&duplicateMenuGroup&id_azmegamenu_group='
					.$azmenugroup['id_azmegamenu_group'].'">
					...
					.Tools::getAdminTokenLite ('AdminModules').'&deleteMenuGroup&id_azmegamenu_group='
					.$azmenugroup['id_azmegamenu_group'].'">
	...
	{
		$this->context->controller->addJS( __PS_BASE_URI__.'modules/azmegamenu/js/jquery.nestable.js' ); 
		$this->context->controller->addCss( __PS_BASE_URI__.'modules/azmegamenu/css/form.css' ); 
		$object   		= new Megamenu();
		$output = '
			<div class="form_content col-md-12 col-lg-12">
				<h3>
					...
					<span class="form-heading-action">
						...
						<a id="desc-product-new" class="list-toolbar-btn btn btn-info" href="'.Context::getContext()->link->getAdminLink('AdminModules').'&configure=azmegamenu&addMenu=1&editMenugroup&id_azmegamenu_group='.(int)Tools::getValue('id_azmegamenu_group').'">
							Add New
						</a>
					</span>
				</h3>';
	...
	{
		$this->context->controller->addJS( __PS_BASE_URI__.'modules/azmegamenu/js/form.js' ); 
		$id_lang    	= $this->context->language->id;
		$id_shop    	= $this->context->shop->id;
		$id_azmegamenu 	= Tools::getValue('id_azmegamenu') ? (int)Tools::getValue('id_azmegamenu') : 0;
		$object   		= new Megamenu($id_azmegamenu);
		...
		$fields_form = array(
			'form' => array(
                ...
                array(
                    'type' => 'switch',
                    'label' => $this->l('Display Menu Title'),
					'hint'  => $this->l('Show/Hide title of menu'),
                    'name' => 'show_title',
                    'values' => $active,
                    'default' => 1,
				...
				array(
					...
					'hint'  => $this->l('Add a label for menu if you want'),
                    'name'  => 'label',
                    'value' => true,
                    'lang'  => true,
                    'default'=> '',
				...
				array(
                    'type' => 'textarea',
                    'label' => $this->l('Menu Description'),
					'hint'  => $this->l('Add a short description for menu if you want'),
                ...
                array(
                    'type' => 'select',
                    'label' => $this->l('Select Menu Parent'),
					'hint'  => $this->l('Select parent of a menu'),
                    'name' => 'id_parent',
				...
				array(
                    'type' => 'switch',
                    'label' => $this->l('Display SubTitle'),
					'hint'  => $this->l('Display subtitle of submenu'),
                    'name' => 'show_sub_title',
                    'values' => $active,
                    'default' => 1,
                ),
                array(
                    'type' => 'select',
                    'label' => $this->l('Select Menu Type'),
					'hint'  => $this->l('Select type of menu'),
                    'name' => 'type',
                    'id'    => 'select_menu',
                ...
                array(
					...
					'hint'  => $this->l('Add ID of product what you want'),
                    'id' => 'type_product',
                    'class'=> 'type_group',
                    'default' => "",
					'desc'    => $this->l('Ex: 3')
                ...
                array(
                    ...
                    'name' => 'menu_class',
					'hint'  => $this->l('Add class for menu if you want'),
                    'display_image' => true,
                    'default' => '',
					'col' => '6',
                ),
				array(
					...
					'hint'  => $this->l('Define width for menu item'),
                    'name' => 'width',
                    'id' => 'width',
                    'class'=> 'width',
                    'default' => "",
				...
				array(
                    'type' => 'select',
                    'label' => $this->l('Have SubMenu'),
					'hint'  => $this->l('Have SubMenu'),
                    'name' => 'sub_menu',
					'options' => array(  'query' => $sub_menu,
                    'id' => 'value',
                    'name' => 'label' ),
				...
				array(
					...
					'hint'  => $this->l('Define width for submenu item'),
                    'name' => 'sub_width',
                    'id' => 'sub_width',
                    'class'=> 'sub_width',
                    'default' => "",
				...
				array(
                    'type' => 'switch',
                    'label' => $this->l('Group SubMenu Item'),
					'hint'  => $this->l('This option to submenu item display inline'),
                    'name' => 'group',
                    'values' => $active,
                    'default' => 0,
                 ),
				array(
                    'type' => 'switch',
                    'label' => $this->l('Active Menu Item'),
					'hint'  => $this->l('Enable/Disable menu item'),
                    'name' => 'active',
                    'values' => $active,
                    'default' => 1,
                 ),
		...
		if (Tools::isSubmit('id_azmegamenu') && $this->menuExists((int)Tools::getValue('id_azmegamenu')))
		{
			$menu = new Megamenu((int)Tools::getValue('id_azmegamenu'));
			$fields_form['form']['input'][] = array('type' => 'hidden', 'name' => 'id_azmegamenu');
		}
		...
		$helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&id_azmegamenu_group='.Tools::getValue('id_azmegamenu_group');
		$helper->token = Tools::getAdminTokenLite('AdminModules');
	...
	public function getAddFieldsValues()
	{
		...
		{
			$mgmenu = new Megamenu((int)Tools::getValue('id_azmegamenu'));
			$fields['id_azmegamenu'] = (int)Tools::getValue('id_azmegamenu', $mgmenu->id);
		}
		else
			$mgmenu = new Megamenu();
	...
	private function getGridItems()
	{
		...
		$id_lang = $this->context->language->id;
		$id_shop = $this->context->shop->id;
		$sql = 'SELECT b.`id_azmegamenu_group`,  b.`hook`, b.`position`, b.`status`, bl.`title`, bl.`content`
			FROM `'._DB_PREFIX_.'azmegamenu_group` b
			LEFT JOIN `'._DB_PREFIX_.'azmegamenu_group_shop` bs ON (b.`id_azmegamenu_group` = bs.`id_azmegamenu_group` )
			LEFT JOIN `'._DB_PREFIX_.'azmegamenu_group_lang` bl ON (b.`id_azmegamenu_group` = bl.`id_azmegamenu_group`)
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			FROM `'._DB_PREFIX_.'azmegamenu_group` cs, `'._DB_PREFIX_.'azmegamenu_group_shop` css
			WHERE css.`id_azmegamenu_group` = cs.`id_azmegamenu_group` AND css.`id_shop` = '.(int)$this->context->shop->id
		);

		return (++$row['next_position']);
	}	
	
	public function moduleExists($id_azmegamenu_group)
	{
		$req = 'SELECT cs.`id_azmegamenu_group` 
				FROM `'._DB_PREFIX_.'azmegamenu_group` cs
				WHERE cs.`id_azmegamenu_group` = '.(int)$id_azmegamenu_group;
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow($req);

		return ($row);
	}
	
	private function getIdMenuByGroup($id_azmegamenu_group,$id_parent=false){
		$sql = '	SELECT ss.`id_azmegamenu`
					FROM `'._DB_PREFIX_.'azmegamenu` ss
					WHERE ss.`id_azmegamenu_group` = '.(int)$id_azmegamenu_group ;
		if($id_parent)	
		$sql .= ' AND id_parent = 1';
	...
	public function duplicateParentMenu($id_parent,$id_azmegamenu,$id_azmegamenu_group){
		$id_lang       = Context::getContext()->language->id;
		$id_shop       = Context::getContext()->shop->id;
		$azmegamenu = new Megamenu();
		$menus         	= $azmegamenu->getChildren($id_parent, $id_lang,$id_shop);
		if($menus){
			foreach($menus as $menu){
				$id_children  = $menu['id_azmegamenu'];
				$parent_menu = new Megamenu((int)$id_children);
				$parent_menu->id = null;
				$parent_menu->id_azmegamenu = null;
				$parent_menu->id_azmegamenu_group = $id_azmegamenu_group;
				$parent_menu->active = 1;
				$parent_menu->id_parent = $id_azmegamenu;
				$parent_menu->position = $parent_menu->getmaxPositonMenu();
				$parent_menu->add();
					$this->duplicateParentMenu($id_children,$parent_menu->id,$id_azmegamenu_group);
			}
		}
	}
}
